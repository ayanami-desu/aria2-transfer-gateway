...
ARIA2_STAGING_DIR=${ARIA2_STAGING_DIR:-./runtime/aria2/staging}
GATEWAY_DATA_DIR=${GATEWAY_DATA_DIR:-./runtime/gateway}
RCLONE_CONFIG_DIR=${RCLONE_CONFIG_DIR:-./runtime/rclone}

mkdir -p "$ARIA2_CONFIG_DIR" "$ARIA2_DOWNLOAD_DIR" "$ARIA2_STAGING_DIR" "$GATEWAY_DATA_DIR" "$RCLONE_CONFIG_DIR"
...
chown -R "$PUID:$PGID" "$ARIA2_CONFIG_DIR" "$ARIA2_DOWNLOAD_DIR" "$ARIA2_STAGING_DIR" "$GATEWAY_DATA_DIR" "$RCLONE_CONFIG_DIR"
